...
// GitHub Actions에서 매일 실행 — ETF 데이터 수집 + Grok AI 분석
...
const ETF_LIST = [
  // 미국 주요
  { symbol: "SPY",  name: "S&P 500",           market: "us", sector: "전체시장" },
  ...
  { symbol: "XLK",  name: "기술 섹터",            market: "us", sector: "기술" },
  ...
  { symbol: "XLY",  name: "임의소비재",            market: "us", sector: "소비재" },
  { symbol: "XLP",  name: "필수소비재",            market: "us", sector: "필수소비재" },
  { symbol: "XLU",  name: "유틸리티",             market: "us", sector: "유틸리티" },
  { symbol: "XLRE", name: "부동산",              market: "us", sector: "부동산" },
  { symbol: "IBB",  name: "바이오텍",             market: "us", sector: "바이오" },
  { symbol: "ARKK", name: "ARK 혁신",            market: "us", sector: "혁신" },
...
async function fetchPrice(symbol) {
  try {
    ...
    // 30일 수익률
    const oldest = validData[0]?.close;
    const ret30d = oldest ? ((price - oldest) / oldest) * 100 : 0;

    // 5일 수익률
    const fiveDayAgo = validData[Math.max(0, validData.length - 6)]?.close;
    ...
    const fromLow = low52 ? ((price - low52) / low52) * 100 : 0;
...
async function getAIRecommendations(etfData) {
  ...
  const summary = etfData
    .filter(e => e.price > 0)
    .map(e => `${e.symbol}(${e.name}): 가격=${e.price.toFixed(2)}, 당일=${e.changePct.toFixed(2)}%, 5일=${e.ret5d.toFixed(2)}%, 30일=${e.ret30d.toFixed(2)}%, 52주고가대비=${e.fromHigh.toFixed(2)}%`)
    .join("\n");
  ...
  const res = await fetch("https://api.x.ai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `당신은 ETF 투자 전문가입니다. 아래 ETF 데이터를 분석해서 JSON으로만 응답하세요. 마크다운 없이 순수 JSON만:
{
  ...
  "updated_at": ""
        ...
        {
          role: "user",
          content: `오늘 ETF 데이터:\n${summary}\n\n각 카테고리별 상위 5개씩 추천해주세요. updated_at은 오늘 날짜(${new Date().toISOString().slice(0,10)})로 설정하세요.`
        }
      ],
      temperature: 0.2,
      max_tokens: 3000,
...
async function main() {
  ...
  for (let i = 0; i < ETF_LIST.length; i += 5) {
    ...
    prices.forEach((price, idx) => {
      if (price) {
        results.push({ ...chunk[idx], ...price });
      }
    });
    if (i + 5 < ETF_LIST.length) {
